...
class FC(Layer):
  ...
  def forward(self, inputs):
    ...
    # TODO
    self.inputs = inputs
    outputs = np.add(np.dot(inputs, self.weights.T), self.bias)
    return outputs
  ...
  def backward_params(self, grads):
    ...
    """
    # TODO
    grad_weights = self.inputs.T.dot(grads).T
...
class SoftmaxCrossEntropyWithLogits():
  ...
  def forward(self, x, y):
    ...
    log_likelihood = -np.sum(y*np.log(probs), axis=1)
...
class L2Regularizer():
  ...
  def forward(self):
    ...
    """
    # TODO
    return 0.5*self.weight_decay*np.sum(np.square(self.weights))
